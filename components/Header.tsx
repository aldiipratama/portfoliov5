  ...
  return (
    <header className='container py-5 fixed top-0 z-[999] dark:bg-black/50 backdrop-filter bg-white/50 backdrop-blur-md shadow-md'>
      <nav className='flex justify-between'>
        <Link href={'/'}>
          <Button variant={'link'} className='p-0'>Aldiipratama</Button>
        </Link>
              ...
              <DropdownMenuContent className={'mr-14 mt-2'}>
                {
                  navbarList.map((navList, i) => (
                    <DropdownMenuItem key={i}>
                      <RSLink to={navList.link} className='w-full' offset={-100}>
                        <Button variant={'ghost'} className='hover:underline'>
                          <span className='dark:text-indigo-500 text-green-500'>#</span>{navList.label}
                        </Button>
                      </RSLink>
                    </DropdownMenuItem>
                  ))
              ...
              navbarList.map((navList, i) => (
                <li key={i}>
                  <Button variant={'link'} className='hover:underline p-0' asChild>
                    <RSLink to={navList.link} className='w-full ' offset={-100} activeClass='active'>
                      <span className='dark:text-indigo-500 text-green-500'>#</span>{navList.label}
                    </RSLink>
                  </Button>
                </li>
